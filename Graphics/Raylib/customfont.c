...
int main(void) {
  ...
  const char msg[50] = "Ligatures? => <=<";

  // NOTE: Textures/Fonts MUST be loaded after Window initialization (OpenGL
  // context is required)

  // TTF Font loading with custom generation parameters
  Font font = LoadFontEx("resources/JetBrainsMonoNFPR.ttf", 96, 0, 0);
  ...
  while (!WindowShouldClose()) { // Detect window close button or ESC key
    ...
    // TODO: It seems texSize measurement is not accurate due to chars
    // offsets...
    // DrawRectangleLines(fontPosition.x, fontPosition.y, textSize.x,
    // textSize.y, RED);

    DrawRectangle(0, screenHeight - 80, screenWidth, 80, LIGHTGRAY);
    DrawText(TextFormat("Font size: %02.02f", fontSize), 20, screenHeight - 50,
             10, DARKGRAY);
    DrawText(
        TextFormat("Text size: [%02.02f, %02.02f]", textSize.x, textSize.y), 20,
        screenHeight - 30, 10, DARKGRAY);
